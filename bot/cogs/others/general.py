...
class General(commands.Cog):
    def __init__(self, bot):
        ...
        self.config = json.load(open(os.getcwd() + '/bot/config/config.json'))  # Updates the config file to make sure we have the most relevant information
    ...
    def cog_check(self, ctx):
        """
        This is called before each command is used; this lets us re-set the config file for any updates
    ...
    async def server_CMD(self, ctx):
        """
        Sends the support server invite like along with the bots' invite link in a discord.Embed()
        :param ctx: Information on the context of where the command was called
        """
